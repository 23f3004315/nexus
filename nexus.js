class NexusAI {
    ...
    saveCurrentConversation() {
        try {
            if (this.state.settings.advanced.autoSave) {
                ...
                localStorage.setItem('nexusai_conversations', JSON.stringify(Array.from(this.state.conversations.entries())));
            }
        } catch (e) {
    ...
    saveAndApplySettings() {
        try {
            ...
            this.applySettings();
            localStorage.setItem('nexusai_settings', JSON.stringify(this.state.settings));
    ...
    async loadSettings() {
        try {
            const stored = localStorage.getItem('nexusai_settings');
            if (stored) {
    ...
    showWelcomeMessage() {
        ...
        if (conv && conv.messages.length === 0) {
            this.addMessage('assistant', 'Welcome to NexusAI! How can I assist you?');
        }
    }
